<template>
  <div class="q-pa-lg dashboard-container">
    <div class="row justify-end">
      <q-card class="my-card q-pa-sm shadow-3 "
    ...
    <q-card-section class="q-pa-md flex justify-center q-gutter-md">
      <div class="column items-center">
        <q-btn color="warning" icon="hourglass_empty" label="Pendientes" :loading=loadingEarrings
          @click="data('earrings'); btnStatus = 'earrings'" style="font-weight: bold;" />
      </div>

      <div class="column items-center">
        <q-btn color="positive" icon="check" label="Aprobadas" :loading=loadingApproved
          @click="data('approved'); btnStatus = 'approved'" style="font-weight: bold;" />
      </div>

      <div class="column items-center">
        <q-btn color="negative" icon="close" label="Rechazadas" :loading=loadingRejected
          @click="data('rejected'); btnStatus = 'rejected'" style="font-weight: bold;" />
      </div>

      <div class="column items-center">
        <q-btn color="positive" icon="download" label="Excel" @click="downloadExcel()" :loading="loading"
    ...
    <q-card>
      ...
      <q-card-section class="row items-center q-col-gutter-md q-pb-none">
        ...
        <!-- Contador -->
        <div class="col-12 col-md-4 text-center">
          <q-badge color="dark" class=" q-pa-sm flex column items-center justify-center bg-gradient shadow-4">
        ...
        <div class="col-12 col-md-4">
          <q-input dense filled v-model="searchDni" label="Buscar por cédula o por nombre" debounce="300" clearable
            type="text" color="dark" />
        </div>
      </q-card-section>

      <q-card-section>
        <q-table separator="cell" :rows="filteredRows" :columns="columns" row-key="_id" flat bordered
          :pagination="{ rowsPerPage: 10 }" class="custom-table">
          ...
          <template v-slot:body-cell-shirt="props">
            <q-td :props="props">
              <q-badge
                ...
                text-color="white" class=" q-pa-sm">
                {{ props.row.shirt }}
              </q-badge>
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-shirtWithSleeves="props">
            <q-td :props="props" class="text-h6">
              <q-icon :name="props.row.shirtWithSleeves === 'Si' ? 'check_circle' : 'cancel'"
                :color="props.row.shirtWithSleeves === 'Si' ? 'green' : 'red'" size="sm" />
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-actions="props">
            <q-td class="text-center">
              <q-btn size="sm" flat round color="info" icon="visibility" @click="verDetalles(props.row)">
                <q-tooltip>Ver detalles</q-tooltip>
              </q-btn>
              <q-btn size="sm" flat round color="green" icon="check"
                :loading="loadingMap.accepted[props.row._id] === true"
                v-show="props.row.state == 2 || props.row.state == 0" class="q-ml-sm "
              ...
              <q-btn size="sm" flat round color="red" icon="close" v-show="props.row.state == 1 || props.row.state == 0"
                :loading="loadingMap.refused[props.row._id] === true" class="q-ml-sm"
        ...
        <q-card-section>
          <div class="q-gutter-md">
              ...
              <div class="col-12 col-sm-6">
                <q-item>
                  ...
                  <q-item-section>
                    ...
                    <q-item-label caption>{{ datosSeleccionados.firstName }} {{ datosSeleccionados.lastName
                      }}</q-item-label>
                  </q-item-section>
                </q-item>
              </div>
              ...
              <div class="col-12 col-sm-6">
                <q-item>
                  ...
                  <q-item-section>
                    ...
                    <q-item-label caption>{{ datosSeleccionados.livesInColombia ? "Colombia" : "Extranjera"
                      }}</q-item-label>
                  </q-item-section>
                </q-item>
              </div>
            ...
            <q-item>
              ...
              <div class="col-12">
                <q-item v-if="datosSeleccionados.rejectionReason">
                  <q-item-section>
                    ...
                    <q-item-label caption class="q-ml-lg text-center">{{ datosSeleccionados.rejectionReason
                      }}</q-item-label>
                  </q-item-section>
                </q-item>
              </div>
              <div class="col-12 col-sm-6">
                <q-item>
                  <q-item-section avatar><q-icon name="chill" color="primary" /></q-item-section>
                  <q-item-section>
                    <q-item-label class="text-bold">Distancia</q-item-label>
                    <q-item-label caption>{{ datosSeleccionados.distance }}</q-item-label>
                  </q-item-section>
                </q-item>
              </div>
              ...
              <div class="col-12 col-sm-6">
                <q-item>
                  ...
                  <q-item-section>
                    ...
                    <q-item-label>
                      <q-img :src="datosSeleccionados.proofImage" alt="Comprobante de pago"
                        style="max-width: 50px;  cursor: pointer;" @click="imagenAmpliada = true" />
                      <div class="q-mt-sm">
                        <a :href="datosSeleccionados.proofImage" target="_blank" rel="noopener noreferrer"
                          class=" text-dark text-caption">Abrir</a>
                      </div>
                    </q-item-label>
                  </q-item-section>
                </q-item>
    ...
    <q-dialog v-model="imagenAmpliada">
      <q-img :src="datosSeleccionados.proofImage" fit="contain" spinner-color="primary" :ratio="1" />
    </q-dialog>
    ...
    <q-dialog v-model="dialogMotivo" persistent>
      <q-card class="q-pa-md bg-grey-1" style="min-width: 400px;">
        ...
        <q-card-section>
          <q-input type="textarea" label="Explica por qué fue rechazado" v-model="motivoRechazo" outlined autogrow
            dense />
        </q-card-section>
...
const columns = computed(() => {
  const baseColumns = [
    ...
    { name: 'distance', label: 'Distancia', field: 'distance', align: 'center' },
